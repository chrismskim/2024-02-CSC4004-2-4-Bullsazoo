...
from backend.backend.test.tts.tts_handler import generate_speech
from backend.backend.test.gpt.gpt_text_generator import gpt_response
...
def detect_objects_realtime(model):
    ...
    if not cap.isOpened():
        generate_speech("웹캠을 찾을 수 없습니다. 웹캠을 장착하시고 시도하세요", None)
        return
...
def main():
    # YOLOv5 모델 경로
    path = str(BASE_DIR) + '/backend/test/yolov5/best.pt'
